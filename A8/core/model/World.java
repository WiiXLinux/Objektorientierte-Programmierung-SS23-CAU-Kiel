...
import A8.core.utils.LabReader;
...
class Tuple<E> {
    /**
     * Inits a tuple of type E with given x and y
     * @param x E x
     * @param y E y
     */
...
public class World {
     ...
     * Here we move every enemy (in the example file there is only 1) to a random free neighbour.
     * Somewhere there is a bug that causes the enemy to not see some possible solutions,
     * but we don't think that the bug is caused here.
     * Also look at it like this: If you play against a bot that jumps sometimes,
     * or doesn't move when it is supposed to, it more or less a feature, since the enemy AI is supposed to be bad ;).
     */
    public void updateField() {
        // First check for a game over.
        // We do this, since updateField is also called after each player move,
        // so we don't want it to be possible for the player to move into the enemy and survive.
        if (field[playerX][playerY] == 2) {
            game_over = true;
            return;
        }
        ...
        for (int i = 0; i < this.width; i++) {
            for (int j = 0; j < this.height; j++) {
                ...
                if (this.field[i][j] == 2) {
                    ...
                    try {
                        UP = this.field[i][j - 1] != 1;
                    } catch (IndexOutOfBoundsException ignore) {
                    }
                    try {
                        LEFT = this.field[i - 1][j] != 1;
                    } catch (IndexOutOfBoundsException ignore) {
                    }
                    try {
                        RIGHT = this.field[i + 1][j] != 1;
                    } catch (IndexOutOfBoundsException ignore) {
                    }
                    try {
                        DOWN = this.field[i][j + 1] != 1;
                    } catch (IndexOutOfBoundsException ignore) {
                    }
